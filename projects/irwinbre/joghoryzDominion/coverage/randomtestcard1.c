...
int main(){
    ...
    printf("+++RANDOM TESTING great_hall+++\n");
    ...
    for (j = 0; j < 25000 ; ++j) {
       ...
       // fflush(stdout);
        ...
        result = cardEffect_great_hall(&endingGameState, 1, handPos);

        if(result != 0){
            printf("-----FAILED: normal run\n");
        }

        if((memcmp(&endingGameState,&startingGameState,sizeof(struct gameState)) ==0)){
            printf("-----FAILED: no state change when expected\n");
        }
        if(endingGameState.handCount[1] != startingGameState.handCount[1]){
            printf("-----FAILED: hand count mismatch\n");
        }
        if(endingGameState.numActions != startingGameState.numActions+1){
    ...
    printf("+++RANDOM TESTING great_hall+++ Completed!\n");
